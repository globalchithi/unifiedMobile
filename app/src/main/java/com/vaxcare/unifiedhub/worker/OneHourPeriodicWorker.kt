...
import com.vaxcare.unifiedhub.di.HourlyJobs
...
@HiltWorker
class OneHourPeriodicWorker @AssistedInject constructor(
    @Assisted context: Context,
    @Assisted params: WorkerParameters,
    override val analyticsRepository: AnalyticsRepository
) : BasePeriodicWorker(context, params) {
    companion object {
        private const val WORKER_NAME = "${BuildConfig.APPLICATION_ID}.WORKER.ONE_HOUR_PERIODIC"
        ...
        fun buildPeriodicWorker(wm: WorkManager, networkConstraints: Constraints): PeriodicWorkRequest {
            val work = PeriodicWorkRequestBuilder<OneHourPeriodicWorker>(1, TimeUnit.HOURS)
