...
import com.vaxcare.unifiedhub.core.ui.TestTags.AddPublic.Complete as testTags

const val CONTENT_PANEL_HEIGHT_PORTRAIT = 760
const val CONTENT_PANEL_HEIGHT_LANDSCAPE = 640

@Composable
fun AddPublicCompleteScreen(navigateBack: () -> Unit, viewModel: AddPublicCompleteViewModel = hiltViewModel()) {
    BaseMviScreen(
        viewModel = viewModel,
        onEvent = { event ->
            when (event) {
                is AddPublicCompleteEvent.NavigateToHome -> {
                    navigateBack()
                }
            }
        }
        ...
        AddPublicCompleteContent(
            state = state,
            onHomeClick = {
                handleIntent(AddPublicCompleteIntent.BackToHome)
            },
            onLogOutClick = {
                handleIntent(AddPublicCompleteIntent.LogOut)
            }
        )
...
@Composable
private fun AddPublicCompleteContent(
    state: AddPublicCompleteState,
    onHomeClick: () -> Unit,
    onLogOutClick: () -> Unit,
) {
    ...
    ProvideStock(state.stockType) {
        val description = stringResource(
            id = R.string.add_public_complete_description,
            state.stockType.prettyName
        )
        TransactionComplete(
...
private fun Default() {
    AddPublicCompleteContent(
        state = AddPublicCompleteSampleData.Default,
        {},
        {}
    )
}
